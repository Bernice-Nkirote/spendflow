...
const guideSections: GuideSection[] = [
  {
    ...
    steps: [
      ...
      "Create users after roles and departments are clear, because each user needs the right role and department for permissions and approvals.",
    ...
    watchFor: [
      ...
      "For a new company, create at least one user who can approve before submitting live records into approval.",
    ],
  },
  {
    id: "business-types",
    ...
    steps: [
      ...
      "For sole proprietorships, review the default approval workflows and keep the owner/admin role on the approval route.",
      "For partnerships, configure partner approval options inside each approval workflow. Partnership options only appear when the workspace business type is Partnership.",
      "For companies, use the normal workflow levels, roles, and departments to route approvals.",
    ],
    watchFor: [
      "Partnership approvals are configurable and not limited to exactly two people.",
      "Partnerships can use the normal approval workflow or require selected partner-role approvals.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Open Permissions and assign or remove only the actions each role needs.",
    ...
    watchFor: [
      ...
      "Permissions are created by the system by default. Admins assign or remove permissions from roles; they do not create new permission names from the UI.",
  ...
  },
  {
    ...
    title: "Users",
    summary:
      "Users are the people who sign in to Tendaflow. Their role, department, and permissions decide what they can create, view, approve, or manage.",
    whoUsesIt: "Admins and company owners.",
    steps: [
      "Open Users from the sidebar.",
      "Create a user with their name, email, phone number where needed, role, and department.",
      "Choose the role based on the work the user should do, such as Procurement, Finance, Approver, Partner, or Admin.",
      "Choose a department that matches the department used in PRs, POs, and approval workflow levels.",
      "Tendaflow emails the user a setup link so they can create their password.",
      "After the user signs in, ask them to test the pages they should access before using the workflow live.",
    ],
    watchFor: [
      "Admin users can access most modules because they manage the system, setup, records, and reports.",
      "Admin users may not see approval queue items unless they are also part of the approval workflow for that record.",
      "If an approver cannot see an item, check their role, department, permissions, and the approval workflow level.",
      "Use separate user accounts for accountability. Do not share one admin account across several people.",
    ],
  },
  {
    ...
    steps: [
      ...
      "Save the workflow and create a small test record to confirm the route behaves as expected.",
    ],
    watchFor: [
      ...
      "Approvers can approve or reject. Rejection requires a comment so the requester knows what to fix.",
    ],
  },
  {
    id: "suppliers",
  ...
  },
  {
    ...
    steps: [
      ...
      "Open the saved PR using View.",
      "Submit for approval when ready.",
      "Cancel the PR if it should no longer move forward.",
    ],
    watchFor: [
      "Admins can create and use PRs when they have access to the module.",
  ...
  },
  {
    ...
    watchFor: [
      "Admins can use the PO module when they have access.",
      "Create PO from approved PR where possible for traceability.",
      "Review suggested suppliers before selecting one.",
      "Invoices can only be created after an approved PO has been sent to the supplier or marked as shared with supplier.",
    ],
  },
  {
    id: "invoices",
    ...
    watchFor: [
      "Admins can create invoices when they have access to the module.",
      "Before creating an invoice, the approved PO should be sent to supplier or marked as shared with supplier.",
      "To send a PO to a supplier: download the approved PDF, sign or stamp it where your organisation requires, upload the signed PDF, then use Send to Supplier.",
      "If you are not emailing through Tendaflow, use Mark as Shared with Supplier where appropriate.",
      "Supplier portal users can create invoices only from eligible POs assigned to them.",
  ...
  },
  {
    ...
    steps: [
      "Open Exchange Rates.",
      "Add or update the currency rate and effective date manually, or sync supported rates from the configured API.",
    ...
    watchFor: [
      ...
      "If finance has approved a manual rate, leave overwrite off unless finance agrees to replace it.",
      "Overwrite existing rates only when the same currency and date should be replaced.",
    ],
  },
  {
    id: "reports",
    ...
    watchFor: [
      ...
      "Supplier lead time should be read as practical time, such as same day, hours, or whole days.",
  ...
  },
  {
    ...
    summary:
      "Tasks and notifications help users find work that needs their attention.",
    whoUsesIt: "Approvers, procurement teams, finance users, and admins.",
    steps: [
      "Watch the Tasks icon for completed upstream work you are allowed to act on, such as creating a PO from an approved PR.",
      "Check the notification bell for items you need to approve.",
      "Open the task or notification to review the related record.",
      "Approve or reject only after checking the details.",
    ],
    watchFor: [
      "Tasks only show actions the user is allowed to perform through their permissions.",
      "Notifications only show approval alerts to users who can approve that workflow item.",
      "A missing task or notification may mean the workflow level, department, role assignment, or permission needs checking.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Supplier users can open the Supplier Portal Guide or Supplier Assistant from the supplier portal navigation.",
    ...
    watchFor: [
      ...
      "The supplier assistant is intentionally supplier-only; it should not expose internal setup, roles, permissions, or admin guidance.",
